...
def read_data_simba(fileparams):
    maps_ = np.load('/home/handrianomena/research/data/Maps_HI_SIMBA_LH_z=0.00.npy')
...
def augment_data_simba(train_x, train_y):
    final_train_data = []
    final_target_train = []
...
def get_simba(train = True):
    maps_list = []
    fileparams = '/home/handrianomena/research/data/params_SIMBA.txt'
    maps_ = np.load('/home/handrianomena/research/data/Maps_HI_SIMBA_LH_z=0.00.npy')
    maps_ = np.log10(maps_) # converting to log10
    ...
    ind_sample = random.sample(range(0, labels.shape[0]), labels.shape[0])
    if train:
        maps_, labels_ = maps[ind_sample][:-3000], labels[ind_sample][:-3000]
        shuffle = True
    else:
...
def get_simba_inv(train = True):
    fileparams = '/home/handrianomena/research/data/params_SIMBA.txt'
    maps, labels = read_data_simba(fileparams)
    ind = 3000
    ...
    if train:
        ...
        train_x, train_y = augment_data_simba(x, y)
